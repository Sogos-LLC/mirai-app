...
function StatusBadge({ status }: { status: SubscriptionStatus }) {
  const statusConfig: Record<SubscriptionStatus, { color: string; label: string }> = {
    [SubscriptionStatus.UNSPECIFIED]: { color: 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300', label: 'Unknown' },
    [SubscriptionStatus.NONE]: { color: 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300', label: 'No Subscription' },
    [SubscriptionStatus.ACTIVE]: { color: 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400', label: 'Active' },
    [SubscriptionStatus.PAST_DUE]: { color: 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400', label: 'Past Due' },
    [SubscriptionStatus.CANCELED]: { color: 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400', label: 'Canceled' },
  };
  ...
  if (error) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Failed to load billing</h3>
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{currentPlan.name} Plan</h3>
          ...
          <CreditCard className="w-8 h-8 text-primary-600 dark:text-primary-400 flex-shrink-0" />
        </div>
        ...
        {plans.map((plan) => {
          ...
          return (
            <div
              ...
              } ${isCurrentPlan ? 'bg-gray-50 dark:bg-dark-50' : 'bg-white dark:bg-dark-surface'}`}
            >
              {plan.popular && (
              ...
              {plan.pricePerSeat ? (
                <div className="mb-3">
                  <span className="text-2xl font-bold text-gray-900 dark:text-white">${plan.pricePerSeat}</span>
                  <span className="text-gray-600 dark:text-gray-400">/seat/month</span>
                </div>
              ) : (
                <div className="mb-3">
              ...
              {isCurrentPlan ? (
                <button
                  disabled
                  className="w-full py-2.5 px-4 rounded-lg bg-gray-100 dark:bg-dark-50 text-gray-500 dark:text-gray-400 font-medium cursor-not-allowed"
                >
                  Current Plan
                </button>
              ) : isEnterprise ? (
                <a
                  ...
                  className="w-full py-2.5 px-4 rounded-lg border border-gray-300 dark:border-dark-border text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-50 dark:hover:bg-dark-50 transition-colors flex items-center justify-center gap-2"
                >
                ...
                <button
                  ...
                  className={`w-full py-2.5 px-4 rounded-lg font-medium transition-colors ${
                    plan.popular
                      ...
                      : 'border border-primary-600 dark:border-primary-500 text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20'
