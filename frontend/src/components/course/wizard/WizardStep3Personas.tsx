...
import { Users, RefreshCw, ChevronDown, ChevronRight, Plus, Check } from 'lucide-react';
...
export function WizardStep3Personas({ context, send }: WizardStep3PersonasProps) {
  ...
  const handleEdit = useCallback((id: string) => {
    const persona = context.smePersonas.find((p) => p.id === id);
    if (persona) setEditingPersona(persona);
  }, [context.smePersonas]);

  const handleSaveEdit = useCallback((persona: SMEPersona) => {
    send({ type: 'UPDATE_SME_PERSONA', persona });
  }, [send]);
  ...
  const addedTemplateIds = new Set(context.smePersonas.map((p) => p.id));

  const handleAddTemplate = useCallback((template: SMETemplate) => {
    if (addedTemplateIds.has(template.id)) return;
    const persona = smeTemplateToPersona(template) as unknown as SMEPersona;
    send({ type: 'ADD_SME_PERSONA', persona });
