...
export function GapTaskList() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setShowSubmitBanner(false), 8000);
    return () => clearTimeout(timer);
  }, [showSubmitBanner]);

  if (isLoading || tasks.length === 0) return null;
  if (allSubmitted && !showSubmitBanner) return null;

  return (
    <>
      {showSubmitBanner && (
        <div className="w-full flex items-center gap-3 rounded-2xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/20 p-4 mb-6">
          <CheckCircle className="w-4 h-4 text-green-600 dark:text-green-400 shrink-0" />
          <span className="flex-1 text-sm font-medium text-green-800 dark:text-green-300">
            Work submitted successfully! The assigner has been notified.
          </span>
          <button
            type="button"
            onClick={() => setShowSubmitBanner(false)}
            className="text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300 transition-colors shrink-0 min-h-[44px] min-w-[44px] flex items-center justify-center"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
