...
export function KnowledgeSourcesModal({
  ...
  const filteredTeamDocs = useMemo(() =>
    teamDocs.filter(
      (doc) =>
        doc.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        doc.summary?.toLowerCase().includes(searchQuery.toLowerCase())
    ),
    [teamDocs, searchQuery]
  );
      ...
      <div className="space-y-4">
        ...
        {activeTab === 'existing' && (
          <div className="space-y-4">
            ...
            {hasTeamDocs && (
              <SourceSection
                icon={<Library className="w-4 h-4 text-muted" />}
                title="Team Knowledge"
                docs={filteredTeamDocs}
                selectedIds={selectedTeamDocIds}
                allCount={teamDocs.length}
                onToggle={onToggleTeamDoc}
                onSelectAll={onSelectAllTeamDocs}
                onDeselectAll={onDeselectAllTeamDocs}
            ...
            {hasGlobalDocs && (
              <SourceSection
                ...
                docs={filteredGlobalDocs}
                selectedIds={selectedGlobalDocIds}
                allCount={globalDocs.length}
                onToggle={onToggleGlobalDoc}
                onSelectAll={onSelectAllGlobalDocs}
                onDeselectAll={onDeselectAllGlobalDocs}
                searchQuery={searchQuery}
                scope="global"
              />
            )}
...
/** Reusable section for team or global knowledge source listing */
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          onClick={selectedIds.length === allCount ? onDeselectAll : onSelectAll}
          className="text-xs text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300 font-medium"
        >
          {selectedIds.length === allCount ? 'Deselect All' : 'Select All'}
        </button>
      </div>
      <div className="space-y-2 max-h-64 overflow-y-auto">
        ...
        {docs.length === 0 && searchQuery && (
          <p className="text-sm text-muted text-center py-4">No {title.toLowerCase()} documents match your search</p>
        )}
      </div>
    </div>
  );
