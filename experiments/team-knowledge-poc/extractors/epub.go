...
// EPUBExtractor handles EPUB files
...
func (e *EPUBExtractor) Extract(ctx context.Context, content []byte, filename string) (*ExtractedDocument, error) {
...
func (e *EPUBExtractor) findFile(files map[string]*zip.File, href string, opfDir string) *zip.File {
	...
	// Also try with URL-decoded path
	for _, p := range paths {
		if f, ok := files[p]; ok {
			return f
		}
...
func (e *EPUBExtractor) extractAllHTMLFiles(files map[string]*zip.File) ([]DocumentSection, strings.Builder) {
	var sections []DocumentSection
	var allText strings.Builder
