...
// pptxSlideContent holds parsed slide data.
...
func (e *PPTXExtractor) Extract(ctx context.Context, content []byte, filename string) (*service.ExtractedDocument, error) {
	...
	for _, file := range reader.File {
		...
		slides = append(slides, pptxSlideContent{
			slideNum: slideNum,
			title:    title,
			text:     text,
		})
	...
	sections := make([]service.DocumentSection, len(slides))
	...
	for i, slide := range slides {
		...
		sections[i] = service.DocumentSection{
			Title:   title,
			Content: slide.text,
			Index:   i,
		}
...
var _ service.DocumentExtractor = (*PPTXExtractor)(nil)
