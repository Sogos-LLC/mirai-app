...
// PDFExtractor implements DocumentExtractor for PDF files.
...
func (e *PDFExtractor) Extract(ctx context.Context, content []byte, filename string) (*service.ExtractedDocument, error) {
	...
	doc := &service.ExtractedDocument{
		...
		Sections:  make([]service.DocumentSection, 0, pageCount),
	}
	...
	for pageNum := 1; pageNum <= pageCount; pageNum++ {
		...
		section := service.DocumentSection{
...
func (e *PDFExtractor) extractTitle(reader *pdf.Reader, sections []service.DocumentSection, filename string) string {
...
func (e *PDFExtractor) extractMetadata(reader *pdf.Reader, doc *service.ExtractedDocument) {
	trailer := reader.Trailer()
	if trailer.IsNull() {
		return
	}
...
// Compile-time check that PDFExtractor implements DocumentExtractor.
var _ service.DocumentExtractor = (*PDFExtractor)(nil)
